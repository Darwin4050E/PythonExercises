"""This module is used to verify the math between a pattern and a string.
"""
...
def answer(question):
    ...
    """
    is_number = re.fullmatch("^What is -?\d+\?$", question)
    is_one_operation = re.fullmatch("^What is -?\d+ (plus|minus|multiplied by|divided by) -?\d+\?$", question)
    is_two_operation = re.fullmatch("^What is -?\d+ (plus|minus|multiplied by|divided by) -?\d+ (plus|minus|multiplied by|divided by) -?\d+\?$", question)
    op = question.strip("What is ?").replace("multiplied by", "multiplied").replace("divided by", "divided").split(" ")
    if is_number: return int(op[0])
    if is_one_operation: return operate(op[1], int(op[0]), int(op[-1]))
    if is_two_operation: return operate(op[3], operate(op[1], int(op[0]), int(op[2])), int(op[-1]))
